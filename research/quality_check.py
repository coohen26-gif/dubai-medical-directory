...
def build_report(dha, dent, sheryan):
    ...
    if dent and dent["phones_checked"] == 0:
        out.append(f"- ℹ️ Dentists `phone`/`email`/`languages`/`photos_url` are all empty in scraped CSV — Sheryan public search doesn't expose them. **Action:** enrich dentists dataset from clinic websites or Google Places API (separate enrichment job).")
