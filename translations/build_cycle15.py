#!/usr/bin/env python3
"""
DMD — Cycle 15/30min (cron 6e3d697a "Dubai - Translation & Localization") :
...
Schema v1.5 (5 langues) — continuité cycle14.

Continuité :
- cycle 14 (5 fiches OralSurg/Pedo/Ortho/Prostho/Gen) → cumul 83 fiches
- ce cycle 15 couvre 5 NOUVELLES fiches, variety specialty + 1 nouvelle nationalité (Cyrillic Russe : Marat Azizov)
- après ce cycle : cumul 88 fiches

Sélection : 5 fiches NON couvertes, variety specialty-driven :
- 1× Oral Surgeon        (Basim Abu Hawas)        — arabe
- 1× Periodontist        (Reneesh Kareem)          — indien (Kerala chrétien)
- 1× Implantologist      (Marat Azizov)            — russe/azeri (Cyrillic test)
- 1× Pediatric Dentist   (Najah Abdelrahman)       — arabe
- 1× General Dentist     (Darya Shahabi)           — iranien (persan)

Livrables :
- translations/per_lang/dentist_{license}_{fr|ar|en|ru|zh}.json (5 × 5 = 25 fichiers)
- translations/fiches-2026-06-06-cycle15.json (résumé)
- translations/build_cycle15.py (script, traçabilité)
"""
import csv
import json
import re
...
OUT_SUMMARY = ROOT / "fiches-2026-06-06-cycle15.json"
