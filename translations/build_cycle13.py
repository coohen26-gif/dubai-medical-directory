#!/usr/bin/env python3
"""
DMD — Cycle 12/30min (cron ad25646f-4b17-4eb0-b148-0b7d037c7231 "Traduction 5 Langues") :
...
Schema v1.5 (5 langues) — continuité cycle07/11.

Continuité : 70 fiches déjà livrées (cycles 1-11) ; ce cycle couvre 5 nouvelles.
Sélection : 5 fiches NON couvertes, variety specialty-driven :
- 1× Endodontist (Mehanas Kasthurikattil)
- 1× Orthodontist (Muawya Alkaseasbeh)
- 1× Pediatric Dentist (Rihab Al Obaidi)
- 1× Periodontist (Yasser Alhindi)
- 1× Prosthodontist (Mohamed Almoselli)

Livrables :
- translations/per_lang/dentist_{license}_{fr|ar|en|ru|zh}.json (5 × 5 = 25 fichiers)
- translations/fiches-2026-06-06-cycle13.json (résumé)
- translations/build_cycle13.py (script, traçabilité)
"""
import csv
import json
import re
...
OUT_SUMMARY = ROOT / "fiches-2026-06-06-cycle13.json"
...
# === Heuristique genre (FR/AR) — set cumulé cycles 1-11 ===
...
MASCULINE_FIRST = {
    ...
    "youssef", "yusuf", "omar", "muawya", "mohamed", "yasser",
}
...
def detect_gender(full_name: str) -> str:
    ...
    if first in MASCULINE_FIRST:
        return "M"
    # Heuristics: ends in 'a' (mostly F in latin/Indian names, but EXCLUDE words
    # that look like other patterns)
    if first.endswith("a") and len(first) > 2:
        return "F"
    # Heuristics: specific M patterns
    if first.endswith(("an", "os", "us", "im", "ar", "al", "ik", "ek", "ok", "uk", "il", "ul", "ed", "ad", "ir", "or", "um", "in", "on", "er", "el", "as", "ir", "iz")):
        # 'as' is ambiguous (M: Mounas/F: Theresas) — only treat as M if name is short
...
# === Translittération nom propre (latin → AR) — table enrichie cycle 12 ===
...
def transliterate_name_ar(full_name: str) -> str:
    ...
    (e.g. 'Al Obaidi' → 'العبيدي' not 'ال عبيدي').
    """
...
# === Translittération nom propre (latin → RU cyrillique) — table enrichie cycle 12 ===
...
RU_NAME_MAP = {
    ...
    "obaidat": "Обаидат",
    "al": "",
    "obaidi": "Обаиди", "obaidy": "Обаиди",
    "kasthurikattil": "Кастурикаттил",
    "mehanas": "Механас",
}

def transliterate_name_ru(full_name: str) -> str:
    """Tokenize and transliterate to Russian cyrillic. Merges 'Al' prefix
    with the next token (e.g. 'Al Obaidi' → 'Аль-Обаиди').
    """
    tokens = full_name.split()
    out_tokens = []
    i = 0
    while i < len(tokens):
        ...
        if key == "al" and i + 1 < len(tokens):
            next_tok = tokens[i + 1]
            next_key = next_tok.lower()
            # Check 'al<next>' first (the al-prefixed entries that already
            # contain the prefix in the source transliteration)
            if f"al{next_key}" in RU_NAME_MAP and RU_NAME_MAP[f"al{next_key}"]:
                val = RU_NAME_MAP[f"al{next_key}"]
                # Only add Аль- prefix if value doesn't already start with it
...
# === Translittération nom propre (latin → ZH pinyin) — table enrichie cycle 12 ===
...
ZH_NAME_MAP = {
    ...
    "obaidat": "Āobāiyīdāt",
    "al": "",
    "obaidi": "Āobāiyīdì", "obaidy": "Āobāiyīdì",
    "kasthurikattil": "Kǎsītǔlīkǎtílí'ěr",
    "mehanas": "Mèhānàsī",
}

def transliterate_name_zh(full_name: str) -> str:
    """Tokenize and transliterate to Hanyu Pinyin diacritique. Merges 'Al'
    prefix with the next token (e.g. 'Al Obaidi' → 'Ā'ěrĀobāiyīdì').
    """
    tokens = full_name.split()
    out_tokens = []
    i = 0
    while i < len(tokens):
        ...
        if key == "al" and i + 1 < len(tokens):
            ...
            if f"al{next_key}" in ZH_NAME_MAP and ZH_NAME_MAP[f"al{next_key}"]:
                ...
                # Only prepend Ā'ěr if value doesn't already start with it
...
def build_bios(gender: str, full_name: str, specialty_fr: str, specialty_ar: str,
    ...
    """Return dict {fr, ar, en, ru, zh} of bio texts."""
    city = CITY.get(city_en, CITY["Dubai"])

    # License text per language + declension
    lt = LICENSE_TYPE.get(license_type_code, LICENSE_TYPE["FTL"])
    lt_ar_inflected = lt["ar"]  # Arabic: no declension
...
def build_one_fiche(row: dict) -> dict:
    """Build the 5-language fiche dict for a single CSV row."""
    ...
    # Look up canonical 5-lang
    if specialty_raw in SPECIALTY:
        sp_data = SPECIALTY[specialty_raw]
    else:
        # Fallback to general-dentist
        sp_data = SPECIALTY["Endodontist"]  # safe default; will overwrite below if needed
        # Actually for safety, build a fallback
        sp_data = {
            "fr": "Dentiste",
            "ar": "طبيب أسنان",
            "en": "Dentist",
...
def emit_per_lang(fiche: dict):
    """Write per_lang/{fr|ar|en|ru|zh}/dentist_{license}_{lang}.json files."""
...
def main():
    # Selection: 5 specific license numbers for cycle 12
    selected_licenses = ["00148580", "00006621", "00004781", "00213213", "00032021"]

    with open(SOURCE, encoding="utf-8") as f:
        reader = csv.DictReader(f)
        rows = list(reader)

    # Enrich with row index
    ...
    summary = {
        "_meta": {
            "schema_version": SCHEMA_VERSION,
            "generated_at": NOW,
            "generator": "build_cycle13.py",
            "cron": "6e3d697a-91cb-4475-872a-8ab965e7ba7f",
            "cron_name": "Dubai - Translation & Localization",
            "cycle": "2026-06-06 cycle 13/30min",
            "previous_cycle": "2026-06-06 cycle 12/30min (75 fiches cumul, 50 in per_lang)",
            "languages": LANGS,
    ...
    print(f"✅ Cycle 13 done: {len(fiches)} fiches × {len(LANGS)} langs = {len(fiches)*len(LANGS)} traductions")
    print(f"   Summary: {OUT_SUMMARY}")
    print(f"   Per-lang files: {len(fiches)*len(LANGS)}")
    print(f"   Specialty mix (cycle 13: Endo/Ortho/Pedo/Perio/Prostho — 5 specialites distinctes): {dict(sp_count)}")
